        ...
        <div className="bg-warm-indigo text-white p-6 md:p-8 rounded-3xl shadow-md">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-white/60 mt-1">Welcome, {user?.name} • System oversight & user management</p>
            </div>
            <button onClick={loadData} className="flex items-center gap-2 bg-card/10 hover:bg-card/20 px-4 py-2.5 rounded-xl text-xs font-bold transition-all">
        ...
        {stats && (
          <div className="bg-card rounded-3xl p-5 border border-border-subtle shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <HealthPill label="API Server" status="healthy" />
              <HealthPill label="Qdrant Vector DB" status={stats.qdrant_status} />
              <HealthPill label="LLM Provider" status={stats.llm_provider} extra="Groq" />
            </div>
          </div>
        )}
